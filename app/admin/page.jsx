...
import { Trash2, ChevronRight, Loader2, Mail, Shield, User, LogOut, Edit, PlusCircle } from "lucide-react";
...
const PermissionCard = ({ name, checked, onChange }) => (
  <div className="flex items-center p-3 bg-white rounded-lg shadow-sm border border-gray-200">
    <input type="checkbox" id={name} name={name} checked={checked} onChange={onChange} className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"/>
    <label htmlFor={name} className="ml-3 block text-sm font-medium text-gray-700 capitalize">{name.replace('_', ' ')}</label>
  </div>
);
...
const UserListItem = ({ user, onDelete, currentUserEmail }) => (
  <div className="flex justify-between items-center p-2 border-b last:border-b-0">
    <div className="flex flex-col">
      ...
      <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full w-fit mt-1">{user.role}</span>
    </div>
    <div className="flex gap-2">
      <Button variant="outline" size="icon" onClick={() => toast.info('Edit user feature coming soon!')}><Edit className="h-4 w-4" /></Button>
      {user.email !== currentUserEmail && (
        <Button variant="destructive" size="icon" onClick={() => onDelete(user.email)}><Trash2 className="h-4 w-4" /></Button>
      )}
    </div>
  </div>
);
...
  };
  ...
  const addMember = async () => {
    ...
    try {
      await axios.post("/api/admin", { email: memberEmail, permissions: memberPermissions, role: memberRole });
  ...
  const renderLoginForm = () => (
    <motion.div initial="hidden" animate="visible" exit="exit" variants={fadeIn} className="space-y-4">
      ...
      <Button onClick={requestOtp} disabled={loading || !email} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
      ...
      <Button onClick={verifyOtp} disabled={loading || !otp} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
  ...
  const renderAdminPanel = () => (
    <motion.div initial="hidden" animate="visible" variants={fadeIn} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={handleLogout} variant="destructive" size="sm" className="bg-red-600 hover:bg-red-700 text-white"><LogOut className="h-4 w-4 mr-2" />Logout</Button>
      </div>
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Quick Actions</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <Link href="/add-event" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Event</Link>
          <Link href="/add-webinar" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Webinar</Link>
          <Link href="/add-gallery" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Gallery</Link>
          <Link href="/add-collaborate" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Collaboration</Link>
          <Link href="/add-sponsor" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Sponsor</Link>
          <Link href="/add-past-events" className={cn(buttonVariants({variant: 'outline'}), "justify-start gap-2")}><PlusCircle/> Add Past Event</Link>
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Add/Update Member</h3>
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="memberRole" className="block text-sm font-medium text-gray-700 mb-1">Assign Role</label>
              <select id="memberRole" value={memberRole} onChange={(e) => setMemberRole(e.target.value)} className="w-full border border-gray-300 rounded-lg py-2 px-3 focus:ring-2 focus:ring-indigo-500">
            ...
            <Button onClick={addMember} className="w-full bg-blue-600 hover:bg-blue-700 text-white">
              {loading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : "Save Member"}
            </Button>
          </div>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {allUsers.length > 0 ? (
              allUsers.map((user) => (<UserListItem key={user._id} user={user} onDelete={deleteMember} currentUserEmail={session.user.email} />))
  ...
  const renderMemberPanel = () => (
    <motion.div initial="hidden" animate="visible" variants={fadeIn} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {session.user.permissions?.event && <Link href="/add-event" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Event</span><ChevronRight /></Link>}
        {session.user.permissions?.collaboration && <Link href="/add-collaborate" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Collaboration</span><ChevronRight /></Link>}
        {session.user.permissions?.sponsor && <Link href="/add-sponsor" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Sponsor</span><ChevronRight /></Link>}
        {session.user.permissions?.webinars && <Link href="/add-webinar" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Webinar</span><ChevronRight /></Link>}
        {session.user.permissions?.gallery && <Link href="/add-gallery" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Gallery</span><ChevronRight /></Link>}
        {session.user.permissions?.past_events && <Link href="/add-past-events" className={cn(buttonVariants(), "justify-between w-full")}><span>Add Past Event</span><ChevronRight /></Link>}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-8">
          ...
          <p className="mt-3 text-gray-500">
            {status === "unauthenticated" ? "Sign in to access your dashboard" : status === "authenticated" && session?.user?.role === "Admin" ? "Manage members and permissions" : "Access your authorized features"}
          </p>
        </motion.div>
